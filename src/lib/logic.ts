...
export function transitionState(
    ...
    if (getCurrentPlayer(state) !== command.player) {
        return {
            ...state,
            error: `It is not ${command.player.name}'s turn. It is ${getCurrentPlayer(state).name}'s turn.`,
        };
    }
    ...
    if (!command.cards) {
        ...
        setCurrentPlayer(nextState, getNextPlayer(command.player, nextState.players));
    ...
    if (getActivePlayers(state).length === 1) {
        ...
        nextState.message = `${command.player.name} won round. Waiting on ${getCurrentPlayer(nextState).name} to start new round.`;
        return nextState;
    }
    ...
    nextState.message = `${getCurrentPlayer(state)} played. Waiting on ${getCurrentPlayer(nextState)}`;
